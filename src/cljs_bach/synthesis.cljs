...
(defn ^:export set-input-value! [input value]
  "Set the value of an input (with immediate effect)."
  (-> input .-offset .-value (set! value)))

(defn ^:export make-input! [context init-value]
  "Create a mutable 'input' (constant source node)."
  (doto (.createConstantSource context)
    (.start)
    (set-input-value! init-value)))

(defn ^:export jack [input]
  "Wrap an input so that it can be used as a source."
  (constantly (source input)))
